...
func TestQuickPublishOnly(t *testing.T) {
	if c := integrationConnection(t, "quick"); c != nil {
		...
		quick.Check(func(msg Publishing) bool {
			_, err := pub.Publish("", queue, false, false, msg)
			return err == nil
		}, nil)
	}
}
...
func TestPublishEmptyBody(t *testing.T) {
	...
	if c != nil {
		...
		_, err = ch.Publish("", queue, false, false, Publishing{})
		if err != nil {
...
func TestPublishEmptyBodyWithHeadersIssue67(t *testing.T) {
	...
	if c != nil {
		...
		_, err = ch.Publish("", queue, false, false, Publishing{Headers: headers})
		if err != nil {
			t.Fatalf("Could not publish")
		}
	...
	if c1 != nil && c2 != nil {
		...
		quick.CheckEqual(
			func(msg Publishing) []byte {
				...
				if _, err := pub.Publish("", queue, false, false, empty); err != nil {
					return []byte{'X'}
				}
				return msg.Body
			},
...
func TestQuickPublishConsumeBigBody(t *testing.T) {
	...
	if c1 != nil && c2 != nil {
		...
		_, err = pub.Publish("", queue, false, false, fixture)
		if err != nil {
...
func TestRoundTripAllFieldValueTypes61(t *testing.T) {
	if conn := integrationConnection(t, "issue61"); conn != nil {
		...
		_, err = ch.Publish("", queue, false, false, Publishing{Body: []byte("ignored"), Headers: headers})
		if err != nil {
...
func TestDeclareArgsRejectToDeadLetterQueue(t *testing.T) {
	if conn := integrationConnection(t, "declareArgs"); conn != nil {
		...
		// Publish the 'poison'
		if _, err := ch.Publish(ex, q, true, false, Publishing{Body: []byte("ignored")}); err != nil {
...
func TestRepeatedChannelExceptionWithPublishAndMaxProcsIssue46(t *testing.T) {
	...
	if conn != nil {
		for i := 0; i < 100; i++ {
			...
			for j := 0; j < 10; j++ {
				_, err = ch.Publish("not-existing-exchange", "some-key", false, false, Publishing{Body: []byte("some-data")})
...
func TestChannelExceptionWithCloseIssue43(t *testing.T) {
	...
	if conn != nil {
		...
		// asynchronous method.
		_, err = c1.Publish("nonexisting-exchange", "", false, false, Publishing{})
		if err != nil {
			panic(err)
		}
...
func TestCorruptedMessageIssue7(t *testing.T) {
	...
	if c1 != nil && c2 != nil {
		...
		for i := 0; i < messageCount; i++ {
			_, err := pub.Publish("", queue, false, false, Publishing{
