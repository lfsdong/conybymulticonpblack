...
func TestPublisher_serve(t *testing.T) {
	...
	ch1 := &mqChannelTest{
		...
		_Publish: func(ex string, key string, mandatory bool, immediate bool, msg amqp.Publishing) (uint64, error) {
			exchangeName = ex
			routingKey = key
			testMsg = &msg
			return 1, nil
...
func TestPublisher_Publish(t *testing.T) {
	...
	go func() {
		...
		envelop.info <- struct {
			published uint64
			err       error
		}{published: 0, err: testErr}
	}()

	_, err := p.Publish(msg1)
	if err != testErr {
